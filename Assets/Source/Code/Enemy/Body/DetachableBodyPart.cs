...
namespace RedRockStudio.SZD.Enemy
{
    public class DetachableBodyPart : IBodyPart
	{
		...
		public DetachableBodyPart(
			IDetachableBone detachableBone, Rigidbody2D rigidbody, IDamageable damageable, int maxHealth)
		{
			_bodyPartImplementation = new BodyPart(rigidbody, damageable, maxHealth);//TODO maybe move to composition root
		...
		public bool HasBeenHalved(bool reset) => 
			_bodyPartImplementation.HasBeenHalved(reset);

		public bool HaveBeenKilled(bool reset) =>
			_bodyPartImplementation.HaveBeenKilled(reset);

		public bool HaveAccumulated(int damage, bool reset) => 
			_bodyPartImplementation.HaveAccumulated(damage, reset);
