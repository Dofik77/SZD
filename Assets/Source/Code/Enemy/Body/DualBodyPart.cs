...
namespace RedRockStudio.SZD.Enemy
{
	public class DualBodyPart : IBodyPart
	{
		...
		public bool HasBeenHalved(bool reset) => 
			_first.HasBeenHalved(reset) || _second.HasBeenHalved(reset);

		public bool HaveBeenKilled(bool reset) =>
			_first.HaveBeenKilled(reset) || _second.HaveBeenKilled(reset);
		...
		private Injuries CalculateInjuries() => 
			(Injuries) Mathf.Max((int) _first.Injuries, (int) _second.Injuries);
	}
}